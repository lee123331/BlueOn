...
const roomUserKey = `${roomId}_${targetUserId}`;
...
await db.query(
  ...
  ON DUPLICATE KEY UPDATE
    ...
    updated_date = CURDATE()
  `,
  [roomId, targetUserId, roomUserKey]
);
...
FROM (
  ...
  /* ===========================
     ...
     - last_msg는 chat_messages에서 최신 1개를 가져옴
  ============================ */
  SELECT
    scr.id AS roomId,
    ...
    /* updated_at은 service_chat_rooms.updated_at 사용 */
